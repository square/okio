...
class PipeAsyncTest {
  ...
  fun test() = runBlocking {
    val pipe = Pipe(6)
    pipe.sink.writeAsync(Buffer().writeUtf8("abc"), 3L)

    val source = pipe.source
    val readBuffer = Buffer()
    assertEquals(3L, source.readAsync(readBuffer, 6L))
    assertEquals("abc", readBuffer.readUtf8())

    pipe.sink.closeAsync()
    assertEquals(-1L, source.readAsync(readBuffer, 6L))
  ...
  fun largeDataset() = runBlocking {
    ...
    val sinkHash = async(dispatcher) {
      ...
      while (i < totalBytes) {
        ...
        buffer.write(data)
        hashingSink.writeAsync(buffer, buffer.size)
        i += data.size.toLong()
      }

      hashingSink.closeAsync()
      hashingSink.hash
    }
    ...
    assertEquals(expectedHash, sinkHash.await())
  ...
  fun sinkTimeout() = runBlocking {
    ...
    val start = now()
    try {
      pipe.sink.writeAsync(Buffer().writeUtf8("def"), 3L)
      fail()
  ...
  fun sinkBlocksOnSlowReader() = runBlocking {
    ...
    val start = now()
    pipe.sink.writeAsync(Buffer().writeUtf8("abcdefghijkl"), 12)
  ...
  fun sinkWriteFailsByClosedReader() = runBlocking {
    ...
    val start = now()
    try {
      pipe.sink.writeAsync(Buffer().writeUtf8("abcdef"), 6)
      fail()
  ...
  fun sinkFlushDoesntWaitForReader() = runBlocking {
    val pipe = Pipe(100L)
    pipe.sink.writeAsync(Buffer().writeUtf8("abc"), 3)
    pipe.sink.flushAsync()
  ...
  @Test @Throws(Exception::class)
  fun sinkMultipleClose() = runBlocking {
    val pipe = Pipe(100L)
    pipe.sink.closeAsync()
    pipe.sink.closeAsync()
  }
  ...
  @Test @Throws(Exception::class)
  fun sourceClose() = runBlocking {
    val pipe = Pipe(100L)
    pipe.source.closeAsync()
    try {
      pipe.source.readAsync(Buffer(), 3)
      fail()
    } catch (expected: IllegalStateException) {
      assertEquals("closed", expected.message)
    }
  ...
  @Test @Throws(Exception::class)
  fun sourceMultipleClose() = runBlocking {
    val pipe = Pipe(100L)
    pipe.source.closeAsync()
    pipe.source.closeAsync()
  }
  ...
  fun sourceBlocksOnSlowWriter()= runBlocking {
    ...
    launch(dispatcher) {
      try {
        ...
        pipe.sink.writeAsync(Buffer().writeUtf8("abc"), 3)
        delay(1000L)
        pipe.sink.writeAsync(Buffer().writeUtf8("def"), 3)
        delay(1000L)
        pipe.sink.writeAsync(Buffer().writeUtf8("ghi"), 3)
        delay(1000L)
        pipe.sink.writeAsync(Buffer().writeUtf8("jkl"), 3)
      } catch (e: IOException) {
        throw AssertionError()
      } catch (e: InterruptedException) {
        throw AssertionError()
