...
class Pipe(internal val maxBufferSize: Long) {
  ...
  internal val mutex = HybridMutex()
  internal val condition = HybridCondition(mutex)
  ...
  val sink = object : Sink {
    ...
    override fun write(source: Buffer, byteCount: Long) {
      ...
      mutex.withLockBlocking(timeout) {
        ...
        while (byteCount > 0) {
          ...
          if (bufferSpaceAvailable == 0L) {
            condition.waitBlocking(timeout) // Wait until the source drains the buffer.
            continue
          }
    ...
    override suspend fun writeAsync(
      ...
      var byteCount = byteCount
      mutex.withLockSuspending(timeout) {
        check(!sinkClosed) { "closed" }
        ...
        while (byteCount > 0) {
          ...
          val bufferSpaceAvailable = maxBufferSize - buffer.size
          if (bufferSpaceAvailable == 0L) {
            condition.waitSuspending(timeout) // Wait until the source drains the buffer.
            continue
          }
  ...
  val source = object : Source {
    ...
    override fun read(sink: Buffer, byteCount: Long): Long {
      mutex.withLockBlocking(timeout) {
        ...
        while (buffer.size == 0L) {
          ...
          condition.waitBlocking(timeout) // Wait until the sink fills the buffer.
        }
    ...
    override suspend fun readAsync(sink: Buffer, byteCount: Long): Long {
      mutex.withLockSuspending(timeout) {
        ...
        while (buffer.size == 0L) {
          if (sinkClosed) return -1L
          condition.waitSuspending(timeout) // Wait until the sink fills the buffer.
        }
