...
internal abstract class Heap : DataStructure() {
  ...
  //   if (head?.left == null) {
  //     println("Heap is empty or has no elements.")
  //     return true
  //   }
  //
  ...
  //     // Check left child
  //     current.left?.let {
  //       if (compareTo(it, current) < 0) {
  //         println("Heap property violated at node $current: left child $it is smaller.")
  //         return false
  //       }
  //       queue.add(it)
  //     }
  ...
  //     current.right?.let {
  //       if (compareTo(it, current) < 0) {
  //         println("Heap property violated at node $current: right child $it is smaller.")
  //         return false
  //       }
  //       queue.add(it)
  //     }
  ...
  //       println("Heap structure violated: node $current has a right child but no left child.")
  //       return false
  //     }
  //
  //     index++
  ...
  // fun checkHeapConsistency(message: () -> String): Boolean {
  //   validateHeap().let {
  //     check(it) {
  //       head?.left?.print()
  //       message()
  ...
  private fun insertAtEnd(node: AsyncTimeout) {
    ...
    if (index % 2 == 0) {
      ...
      // check(
      //   current!!.left == null,
      // ) { "Current left child is not null. Heap size: $heapSize, index: $index, current: $current" }
      current!!.left = node
    } else {
      // Insert as the right child.
      // check(
      //   current!!.right == null,
      // ) { "Current right child is not null. Heap size: $heapSize, index: $index, current: $current" }
      current!!.right = node
    }
    node.parent = current
    heapSize = newSize
  ...
  private fun findLast(): AsyncTimeout? {
    ...
    // check(current!!.left == null && current.right == null) { "This is not a leaf node" }
