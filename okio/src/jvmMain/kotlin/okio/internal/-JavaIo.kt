...
internal fun OutputStream.write(source: Buffer, byteCount: Long, timeout: Timeout) {
  ...
  while (remaining > 0L) {
    ...
    write(head.data, head.pos, toCopy)
...
internal fun InputStream.read(sink: Buffer, byteCount: Long, timeout: Timeout): Long {
  ...
  try {
    ...
    val bytesRead = read(tail.data, tail.limit, maxToCopy)
    if (bytesRead == -1) {
