 ...
 * ```
 */
actual class HashingSource : ForwardingSource, RawSource { // Need to explicitly declare source pending fix for https://youtrack.jetbrains.com/issue/KT-20641
  private val messageDigest: MessageDigest?
  private val mac: Mac?

  internal constructor(source: RawSource, digest: MessageDigest) : super(source) {
    this.messageDigest = digest
    this.mac = null
  }

  internal constructor(source: RawSource, algorithm: String) : this(source, MessageDigest.getInstance(algorithm))

  internal constructor(source: RawSource, mac: Mac) : super(source) {
    this.mac = mac
    this.messageDigest = null
  }

  internal constructor(source: RawSource, key: ByteString, algorithm: String) : this(
    source,
    try {
  ...
  actual companion object {
     ...
     */
    @JvmStatic
    actual fun md5(source: RawSource) = HashingSource(source, "MD5")
     ...
     */
    @JvmStatic
    actual fun sha1(source: RawSource) = HashingSource(source, "SHA-1")

    /** Returns a source that uses the SHA-256 hash algorithm to produce 256-bit hashes. */
    @JvmStatic
    actual fun sha256(source: RawSource) = HashingSource(source, "SHA-256")

    /** Returns a source that uses the SHA-512 hash algorithm to produce 512-bit hashes. */
    @JvmStatic
    actual fun sha512(source: RawSource) = HashingSource(source, "SHA-512")

    /** Returns a source that uses the obsolete SHA-1 HMAC algorithm to produce 160-bit hashes. */
    @JvmStatic
    actual fun hmacSha1(source: RawSource, key: ByteString) = HashingSource(source, key, "HmacSHA1")

    /** Returns a source that uses the SHA-256 HMAC algorithm to produce 256-bit hashes. */
    @JvmStatic
    actual fun hmacSha256(source: RawSource, key: ByteString) = HashingSource(source, key, "HmacSHA256")

    /** Returns a source that uses the SHA-512 HMAC algorithm to produce 512-bit hashes. */
    @JvmStatic
    actual fun hmacSha512(source: RawSource, key: ByteString) = HashingSource(source, key, "HmacSHA512")
  }
}
